...
const StatusIcon = ({actType}) => {
	return actType === "listening" ? (
...
const StatusPreview = ({
	actType, appName="AppName",
	user, user_avatar,
	state, details, status_type="name",
}) => {
	return (
		<div className="bg-[#3f4048] text-[#dfe0e2] flex items-center gap-3 rounded-lg py-1 px-2 select-none">
			<div className="relative">
				<img className="h-8 w-8 object-cover rounded-full select-none"
					draggable={false} src={user_avatar || "assets/avatar-blue.png"}
					onError={({ currentTarget }) => {
						currentTarget.src = "assets/avatar-blue.png";
					}}
				/>
				<span className="bg-[#45a366] absolute h-3 w-3 rounded-full bottom-0 right-0 border-2 border-[#3f4048]"></span>
			</div>
			<div className="flex flex-col gap-0.5">
				<span className="text-sm">{user || <T>header_user_default_name</T>}</span>
				<span className="text-xs flex gap-1 items-center">
					<StatusIcon actType={actType}/>
					<span>
						{
							(status_type == "state" && state) ? state :
							(status_type == "details" && details) ? details :
							appName
						}
					</span>
				</span>
